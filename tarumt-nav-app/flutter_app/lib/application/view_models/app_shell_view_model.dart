...
enum AppSection { home, navigate, liveMap, saved, settings }
...
final class AppShellViewModel {
  ...
  void selectSection(AppSection section, {bool resetNavigatePage = false}) {
    _throwIfDisposed();
    final navigatePage = resetNavigatePage
        ? AppNavigatePage.selectFloor
        : _state.navigatePage;
    if (_state.selectedSection == section &&
        _state.navigatePage == navigatePage) {
      return;
    }
    _emit(
      AppShellViewState(navigatePage: navigatePage, selectedSection: section),
    );
  }
