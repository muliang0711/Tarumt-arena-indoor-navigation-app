...
final class FloorRoomsViewModel {
  ...
  /// Clears transient navigation choices while retaining the last floor as a
  /// convenience and preserving the independently managed saved-room list.
  void resetNavigationDraft() {
    _throwIfDisposed();
    if (_state.filter == FloorRoomFilter.all &&
        _state.query.isEmpty &&
        _state.selectedRoomId == null) {
      return;
    }
    _emit(filter: FloorRoomFilter.all, query: '', replaceSelectedRoom: true);
  }
