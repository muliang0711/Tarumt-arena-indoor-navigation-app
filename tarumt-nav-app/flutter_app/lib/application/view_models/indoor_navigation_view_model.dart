...
final class IndoorNavigationViewModel {
  ...
  final WifiDiagnosticLog _wifiDiagnosticLog;
  ...
  double _zoom = indoorNavigationZoomAt(indoorNavigationDefaultZoomIndex);
  int _generation = 0;
  ...
  double? _navigationMapHeadingDegrees;
  ...
  Future<void> _startNavigation(_NavigationDestination requested) async {
    ...
    if (isNewSession) {
      ...
      _navigationMapHeadingDegrees = null;
  ...
  MapBootstrapData _createRuntimeData(
    ...
    return MapBootstrapData(
      bundleRevision: source.bundleRevision,
  ...
  void _createRuntime(MapBootstrapData data) {
    ...
    final rawMotion = RawMotionPdrEngine(
      clock: _clock,
      initialHeadingCorrectionDegrees: mapHeadingFromDeviceHeadingDegrees(0),
      initialRedMarker: data.mapModel.redMarker,
    ...
    if (wifiEngine != null) {
      final coordinator = WifiPositioningCoordinator(
        ...
        diagnosticLog: _wifiDiagnosticLog,
  ...
  WifiFusionContext? _createWifiFusionContext() {
    ...
    return WifiFusionContext(
      ...
      routePath: data.mapModel.routePath,
      wrongWayDetected:
          _wrongWayMonitor?.state.result.shouldSuggestReroute ?? false,
    );
  }
  ...
  Future<void> _applyWifiCorrection(WifiCorrectionDecision decision) async {
    ...
    final recalculatesRoute =
        trustedNode.nodeId != destination.destinationNodeId;
    _wifiCorrectionVisual = WifiCorrectionVisualState(
      fromPosition: fromPosition,
      kind: decision.kind,
      phase: WifiCorrectionVisualPhase.recalculating,
      recalculatesRoute: recalculatesRoute,
      sequence: ++_wifiCorrectionSequence,
      toPosition: provisionalTarget,
    );
    _rebuildReadyState();

    if (recalculatesRoute) {
      await Future<void>.delayed(
        const Duration(milliseconds: wifiRouteRecalculationDelayMs),
      );
      if (_disposed || _activeDestination != destination) return;
    }
    ...
    _wifiCorrectionVisual = WifiCorrectionVisualState(
      fromPosition: fromPosition,
      kind: decision.kind,
      phase: WifiCorrectionVisualPhase.routeReady,
      recalculatesRoute: recalculatesRoute,
      sequence: _wifiCorrectionSequence,
  ...
  void zoomIn() {
    _requireReady();
    setZoom(nextIndoorNavigationZoomIn(_zoom));
  }

  void zoomOut() {
    _requireReady();
    setZoom(nextIndoorNavigationZoomOut(_zoom));
  }
  ...
  /// Cancels the current user-requested navigation without disposing the
  /// reusable map runtime. Unlike [pause], this deliberately forgets the
  /// destination and session so returning to Navigate starts a new journey.
  Future<void> cancelNavigation({String reason = 'user_left_map'}) {
    _ensureNotDisposed();
    return _enqueueNavigation(() => _cancelNavigation(reason));
  }

  Future<void> _cancelNavigation(String reason) async {
    if (_disposed) return;
    final destination = _activeDestination ?? _requestedDestination;
    final hadNavigation =
        destination != null ||
        _activeNavigationSessionId != null ||
        _navigationSessionStatus != null;
    if (!hadNavigation) return;
    ...
    if (_state.isReady) {
      ...
      try {
        ...
        final bootstrapData = _bootstrapEngine.state.data;
        if (bootstrapData != null) {
          final data = _createRuntimeData(bootstrapData, null);
          _runtimeData = data;
          _routeSimulationEngine!.replaceRoutePath(data.mapModel.routePath);
          final routePosition = _routeSimulationEngine!.state.routePosition;
          _derivedEstimateEngine!
  ...
  Future<void> pause({String reason = 'unspecified'}) {
    _requireReady();
    return _enqueueLifecycle(() => _pauseLifecycle(reason));
  }

  Future<void> _pauseLifecycle(String reason) async {
    ...
    _wifiPositioningCoordinator?.pause(reason: reason);
    _synchronizing = true;
    try {
  ...
  Future<void> resume({String reason = 'unspecified'}) {
    _requireReady();
    return _enqueueLifecycle(() => _resumeLifecycle(reason));
  }

  Future<void> _resumeLifecycle(String reason) async {
    if (_disposed || !_state.isReady) {
      return;
    }
    if (_lifecycleStatus != IndoorNavigationLifecycleStatus.paused) {
    ...
    try {
      ...
      if (_navigationSessionStatus == NavigationSessionStatus.navigating) {
        _wifiPositioningCoordinator?.resume(reason: reason);
      }
    } finally {
      _resumeRawMotion = false;
      _resumeSimulation = false;
  ...
  void _updateHeading(double headingDegrees) {
    ...
    if (_navigationSessionStatus == NavigationSessionStatus.navigating) {
      _navigationMapHeadingDegrees = headingDegrees;
    }
    _synchronizing = true;
    try {
  ...
  void _rebuildReadyState() {
    ...
    _emit(
      IndoorNavigationViewState(
        ...
        navigationMapHeadingDegrees: _navigationMapHeadingDegrees,
