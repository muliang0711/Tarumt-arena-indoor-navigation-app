...
final class _Map extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final navigationDisplayHeading =
        state.navigationSessionStatus == NavigationSessionStatus.navigating ||
            state.navigationSessionStatus == NavigationSessionStatus.arrived
        ? state.navigationDisplayHeadingDegrees
        : state.derivedEstimate!.observedHeadingDegrees;
    return Stack(
      fit: StackFit.expand,
      children: [
        IndoorMapViewport(
          ...
          mapImage: state.bootstrap!.mapImage,
        ...
        if (!isEdgeEditing &&
            state.wifiCorrectionVisual?.recalculatesRoute == true)
          Positioned(
            left: 12,
            right: 12,
            top: room != null ? 108 : 12,
            child: WifiRouteRecalculationBanner(
              correction: state.wifiCorrectionVisual!,
            ),
          )
        else if (!isEdgeEditing &&
            (state.wifiPositioning.isActionableFailure ||
                state.wifiPositioning.isLocatingInitialPosition))
          Positioned(
            left: 12,
            right: 12,
            top: room != null ? 108 : 12,
