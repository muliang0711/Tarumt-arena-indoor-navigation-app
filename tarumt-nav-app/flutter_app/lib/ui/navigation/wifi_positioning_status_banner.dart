...
final class WifiPositioningStatusBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (!state.isActionableFailure && !state.isLocatingInitialPosition) {
      return const SizedBox.shrink();
    }
    final content = state.isLocatingInitialPosition
        ? const _WifiBannerContent(
            background: Color(0xFF1D4ED8),
            canRetry: false,
            detail: 'Scanning nearby access points before the first PDR fix.',
            icon: Icons.my_location,
            title: 'Finding your initial position',
          )
        : _contentFor(state.phase);
    return Semantics(
      container: true,
      liveRegion: true,
...
_WifiBannerContent _contentFor(WifiPositioningPhase phase) => switch (phase) {
  ...
  WifiPositioningPhase.throttled => const _WifiBannerContent(
    canRetry: false,
    detail: 'Android paused scanning. Retrying in 5 seconds; PDR stays active.',
...
final class _WifiBannerContent {
  const _WifiBannerContent({
    this.background = const Color(0xFF92400E),
    this.canRetry = true,
