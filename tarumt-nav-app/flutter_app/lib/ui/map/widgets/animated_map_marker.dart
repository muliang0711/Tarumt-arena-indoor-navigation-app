...
final class AnimatedMapMarker extends StatefulWidget {
  const AnimatedMapMarker({
    ...
    this.teleportThresholdPixels = double.infinity,
    super.key,
  });
  ...
  final double teleportThresholdPixels;
...
final class _AnimatedMapMarkerState extends State<AnimatedMapMarker>
  ...
  void didUpdateWidget(covariant AnimatedMapMarker oldWidget) {
    ...
    final distance = math.sqrt(
      math.pow(nextLeft - _animatedLeft.value, 2) +
          math.pow(nextTop - _animatedTop.value, 2),
    );
    if (distance > widget.teleportThresholdPixels) {
      _positionController.stop();
      _animatedLeft = AlwaysStoppedAnimation(nextLeft);
      _animatedTop = AlwaysStoppedAnimation(nextTop);
