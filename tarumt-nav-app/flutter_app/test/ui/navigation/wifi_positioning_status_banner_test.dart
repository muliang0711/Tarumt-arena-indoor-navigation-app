...
void main() {
  ...
  testWidgets('explains the initial Wi-Fi fix before PDR correction', (
    tester,
  ) async {
    await _pump(tester, WifiPositioningPhase.checkingAccess);

    expect(find.text('Finding your initial position'), findsOneWidget);
    expect(
      find.text('Scanning nearby access points before the first PDR fix.'),
      findsOneWidget,
    );
  ...
  testWidgets('shows automatic throttle recovery without a retry button', (
    ...
    expect(
      find.text(
        'Android paused scanning. Retrying in 5 seconds; PDR stays active.',
      ),
      findsOneWidget,
    );
    expect(find.byKey(WifiPositioningStatusBannerKeys.retry), findsNothing);
