...
void main() {
  testWidgets('decorates any map with removable collapsed test controls', (
    ...
    expect(
      tester
          .getSize(find.byKey(WifiPositioningMapTestOverlayKeys.expanded))
          .height,
      lessThanOrEqualTo(600 * 0.34),
    );
    ...
    expect(
      find.byKey(WifiPositioningMapTestOverlayKeys.compact),
      findsOneWidget,
    );
    expect(find.text('node-1 → node-1'), findsOneWidget);

    await tester.tap(find.byKey(WifiPositioningMapTestOverlayKeys.compact));
    await tester.pump();
    expect(
      find.byKey(WifiPositioningMapTestOverlayKeys.expanded),
      findsOneWidget,
    ...
    await tester.tap(find.byKey(WifiPositioningMapTestOverlayKeys.minimize));
    await tester.pump();
    expect(find.byKey(WifiPositioningMapTestOverlayKeys.open), findsOneWidget);
