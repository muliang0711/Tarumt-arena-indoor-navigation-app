...
void main() {
  ...
  testWidgets('fixed-map compass heading drives Bob and cone together', (
    ...
    final northHeading = mapHeadingFromDeviceHeadingDegrees(0);
    await tester.pumpWidget(
      _host(
        UserPresenceMarker(
          observedHeadingDegrees: northHeading,
    ...
    expect(_assetName(tester), endsWith('idle_down.png'));
    expect(
      tester.widget<UserViewCone>(find.byType(UserViewCone)).headingDegrees,
      90,
    );

    final eastHeading = mapHeadingFromDeviceHeadingDegrees(90);
    await tester.pumpWidget(
      _host(
        UserPresenceMarker(
          observedHeadingDegrees: eastHeading,
          position: _position(0),
        ),
      ),
    );
    expect(_assetName(tester), endsWith('idle_left.png'));
    expect(
      tester.widget<UserViewCone>(find.byType(UserViewCone)).headingDegrees,
      180,
    );
