...
void main() {
  ...
  testWidgets('opens Live Map without starting navigation sensors', (
    tester,
  ) async {
    final harness = _createHarness(mapJson: mapJson, edgesJson: edgesJson);
    await tester.pumpWidget(
    ...
    expect(harness.liveScheduler.activeTaskCount, 1);
    expect(
      harness.navigationViewModel.state.loadStatus,
      IndoorNavigationLoadStatus.idle,
    );
    expect(harness.sensor.startCallCount, 0);
  ...
  testWidgets('a new navigation clears the previous manual Wi-Fi session', (
    tester,
  ) async {
    ...
    final wifiTestLabViewModel = WifiPositioningTestLabViewModel(
      ...
      validationCatalog: WifiValidationCatalog(<WifiValidationSample>[
        WifiValidationSample(
          ...
          scanId: 1,
          sessionId: 'old-navigation',
          timestampMs: 1000,
        ),
      ]),
    );
    await wifiTestLabViewModel.submitRandomSample('node-1');
    expect(wifiManager.readings, isNotEmpty);
    expect(
      wifiTestLabViewModel.state.phase,
      WifiPositioningTestLabPhase.success,
    );

    harness.shellViewModel.openFloorRooms();
    await tester.pumpWidget(
      IndoorNavigationApp(
        floorRoomsViewModel: harness.floorRoomsViewModel,
        liveMapViewModel: harness.liveMapViewModel,
    ...
    await tester.ensureVisible(
      find.byKey(FloorRoomsScreenKeys.navigate('elevator')),
    );
    await tester.tap(find.byKey(FloorRoomsScreenKeys.navigate('elevator')));
    ...
    expect(harness.floorRoomsViewModel.state.selectedRoom?.id, 'elevator');
    expect(
  ...
  testWidgets('production room starts its route, Bob, cone, and real sensors', (
    ...
    expect(harness.presenceRepository.leaveCount, 1);
    expect(harness.presenceRepository.disconnectCount, 0);
...
_AppShellHarness _createHarness({
  ...
  final liveMapAssetRepository = FakeMapAssetRepository()
    ..enqueueTiledMapJson(
      assetPath: 'assets/maps/demo_1.tmj.json',
      json: mapJson,
    )
...
final class _AppShellHarness {
  const _AppShellHarness({
    ...
    required this.liveScheduler,
    required this.presenceCoordinator,
  ...
  final FakePeriodicScheduler liveScheduler;
  final RealtimePresenceCoordinator presenceCoordinator;
  final _RecordingPresenceRepository presenceRepository;
