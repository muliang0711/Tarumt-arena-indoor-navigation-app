...
void main() {
  ...
  test('restores the initial floor heading correction on reset', () async {
    final headings = <double>[];
    final harness = _Harness(
      initialHeadingCorrectionDegrees: 90,
      onHeading: headings.add,
    );
    await harness.engine.start();

    harness.fakeSensor.emit(_heading(80));
    await _flushAsync();
    harness.engine.setHeadingCorrectionDegrees(180);
    ...
    expect(headings, <double>[170, 260, 170]);
    await harness.dispose();
  });
...
final class _Harness {
  _Harness({
    ...
    engine = RawMotionPdrEngine(
      clock: clock,
      initialHeadingCorrectionDegrees: initialHeadingCorrectionDegrees,
