  ...
  test(
    ...
    () async {
      final client = _FakeWifiScanChannelClient()
        ...
        ..enqueue(WifiScanChannelContract.scanMethod, <String, Object?>{
          ..._scanResponse,
          'source': 'cached',
        })
      ...
      final scan = await manager.scan();
      expect(scan.source, WifiScanBatchSource.active);
      expect(scan.readings.single.rssi, -55);
      expect(
        (await manager.scan(requestActiveScan: false)).source,
      ...
      expect(
        ...
        <String>['checkAccess', 'requestPermission', 'scan', 'scan', 'dispose'],
      );
      expect(
        client.invocations.map((invocation) => invocation.arguments),
        <Map<String, Object?>?>[
          <String, Object?>{'schemaVersion': 3},
          <String, Object?>{'schemaVersion': 3},
          <String, Object?>{'schemaVersion': 3, 'requestActiveScan': true},
          <String, Object?>{'schemaVersion': 3, 'requestActiveScan': false},
          <String, Object?>{'schemaVersion': 3},
        ],
      );
      await expectLater(
